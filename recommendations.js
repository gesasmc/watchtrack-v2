/* WatchTrack v3.3 – „Für euch“ mit Vorlieben + dauerhaftem Nicht-interessiert-Feedback */
(() => {
  ...
  fetchDiscover=async function(type,category,page){if(category==='foryou')return wtForYou(type);return baseFetchDiscover(type,category,page);};
  ...
  renderCategoryChips=function(){
    ...
    if(state.discoverCategory==='foryou'){
      ...
      if(el)el.textContent=librarySeeds.length?`Basierend auf eurer Liste${rated?` · ${rated} Vorlieben`:''}${hidden?` · ${hidden} nicht interessiert`:''}`:`Fügt erst ${state.discoverType==='tv'?'Serien':'Filme'} zu eurer Liste hinzu`;
    }
  };

  function buttonHtml(type,id,title){
    return `<button class="rec-hide-btn" type="button" data-rec-hide="${type}:${id}" data-rec-title="${esc(title)}" aria-label="${esc(title)} nicht mehr vorschlagen">👎 <span>Will ich nicht sehen</span></button>`;
  }
  ...
  card=function(x,type){
    ...
    return html.replace('</article>',`${buttonHtml(type,x.id,titleOf(x))}</article>`);
  };

  function decorateForYouCards(){
    if(state.discoverCategory!=='foryou')return;
    document.querySelectorAll('#discoverContent .media-card').forEach(el=>{
      if(el.querySelector('[data-rec-hide]'))return;
      const key=el.dataset.open||'';
      const [type,id]=key.split(':');
      if(!type||!id)return;
      const item=(state.discoverItems||[]).find(x=>String(x.id)===String(id));
      const title=item?titleOf(item):(el.querySelector('.media-title')?.textContent||'Dieser Titel');
      el.insertAdjacentHTML('beforeend',buttonHtml(type,id,title));
    });
  }

  const baseLoadDiscover=loadDiscover;
  loadDiscover=async function(){
    const result=await baseLoadDiscover.apply(this,arguments);
    decorateForYouCards();
    return result;
  };

  function saveFeedback(){
    localStorage.setItem(FEEDBACK_STORE,JSON.stringify(state.recommendationFeedback||{}));
    if(typeof saveLibrary==='function')saveLibrary();
  }
  function rejectRecommendation(type,id,title){
    const key=`${type}:${id}`;
    state.recommendationFeedback[key]={id:Number(id),type,title:title||'',hidden:true,preference:'dislike',updatedAt:Date.now()};
    saveFeedback();
  }
  ...
  document.addEventListener('click',e=>{
    ...
    rejectRecommendation(type,id,btn.dataset.recTitle||'');
    btn.closest('.media-card')?.remove();
    renderCategoryChips();
    toast('Vorgemerkt – dieser Titel wird nicht mehr empfohlen');
  },true);

  setTimeout(decorateForYouCards,0);
})();
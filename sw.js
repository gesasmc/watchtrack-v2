const CACHE='watchtrack-v3.0-update-fix';
const SHELL=['./','./index.html','./styles.css','./features.css','./custom.css','./v24.css','./sync.css','./notifications.css','./next-up.css','./app.js','./features.js','./custom.js','./v24.js','./sync.js','./series-first.js','./stable-progress.js','./notifications.js','./next-up.js','./recommendations.js','./upcoming-library.js','./v3.js','./manifest.webmanifest','./icons/icon-180.png','./icons/icon-192.png','./icons/icon-512.png'];
...
self.addEventListener('fetch',e=>{
 ...
 if(isNav||isAsset){
   e.respondWith(fetch(e.request,{cache:'no-store'}).then(resp=>{if(resp.ok&&e.request.method==='GET'){const copy=resp.clone();caches.open(CACHE).then(c=>c.put(e.request,copy));}return resp;}).catch(()=>caches.match(e.request).then(r=>r||caches.match('./index.html'))));
   return;
 }
 e.respondWith(caches.match(e.request).then(cached=>cached||fetch(e.request).then(resp=>{if(resp.ok&&e.request.method==='GET'){const copy=resp.clone();caches.open(CACHE).then(c=>c.put(e.request,copy));}return resp;}).catch(()=>caches.match('./index.html'))));
});

...
(()=>{
  function mount(){
    ...
    const overview=document.createElement('div');overview.className='wt-overview';overview.innerHTML='<div class="wt-overview-card"><strong>Für euch</strong><span>Persönliche Tipps</span></div><div class="wt-overview-card"><strong>Demnächst</strong><span>Starts & Folgen</span></div><div class="wt-overview-card"><strong>Meine Liste</strong><span>Alles gespeichert</span></div>';switcher.after(overview);
    const sport=document.createElement('div');sport.id='wtSportPlaceholder';sport.className='wt-sport-placeholder';sport.innerHTML='<h2>Sport</h2><p>Hier entsteht dein persönlicher Sportbereich. Teams, Vereine, Kämpfer und ganze Ligen/Organisationen können später separat pro Person abonniert werden.</p><div class="wt-sport-preview"><div class="wt-sport-row"><span>⚽</span><div><b>Meine Vereine & Teams</b><small>Spiele und Termine auf einen Blick</small></div></div><div class="wt-sport-row"><span>🥊</span><div><b>Ligen & Kampftage</b><small>Events statt nur einzelne Sportler</small></div></div><div class="wt-sport-row"><span>🔔</span><div><b>Meine Benachrichtigungen</b><small>Persönlich – nicht automatisch für die gemeinsame Liste</small></div></div></div>';overview.after(sport);
    function select(section){
      switcher.querySelectorAll('[data-wt-section]').forEach(b=>b.classList.toggle('active',b.dataset.wtSection===section));
      const isSport=section==='sport';sport.classList.toggle('active',isSport);overview.style.display=isSport?'none':'';
      [...discover.children].forEach(el=>{if([switcher,overview,sport].includes(el))return;el.style.display=isSport?'none':'';});

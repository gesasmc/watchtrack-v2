...
const BOXERS=[
{id:'agit-kabayel',name:'Agit Kabayel',aliases:['Kabayel','Agit'],country:'Deutschland',badge:'🥊'}
];
function boxerResult(b){return{id:b.id,type:'fighter',provider:'public-boxing',name:b.name,sport:'Boxen',country:b.country,badge:b.badge||'🥊',theme:'neutral'}}
function search(q){const n=norm(q);if(['verein','vereine','bundesliga','fussball','team','teams'].includes(n))return TEAMS.filter(t=>!t.national).map(t=>result(t));const out=TEAMS.map(t=>({t,s:score(t,n)})).filter(x=>x.s).sort((a,b)=>b.s-a.s).map(x=>result(x.t));BOXERS.forEach(b=>{const hay=[b.name,...(b.aliases||[])].map(norm);if(hay.some(v=>v.includes(n)||n.includes(v)))out.push(boxerResult(b))});if(n.includes('oktagon')||'oktagonmma'.includes(n))out.push({id:'oktagon',type:'league',provider:'public-oktagon',name:'OKTAGON MMA',sport:'MMA',country:'Europa'});return out.slice(0,20)}
const VISUALS={
...
export async function onRequestGet({request}){const u=new URL(request.url),action=u.searchParams.get('action')||'',id=u.searchParams.get('id')||'',q=(u.searchParams.get('q')||'').trim(),provider=u.searchParams.get('provider')||'';try{if(action==='status')return json({football:true,combat:true,provider:'public-web'});if(action==='search')return json({results:q.length<2?[]:search(q)});if(action==='league-next'&&(provider==='public-oktagon'||id==='oktagon')){const events=OKTAGON.filter(e=>new Date(e.strTimestamp)>Date.now()-86400000);for(const e of events){e.poster=await ogImage(e.sourceUrl)}return json({events});}if(action==='fighter-next'&&provider==='public-boxing'){
  const b=BOXERS.find(x=>x.id===id); if(!b)return json({error:'Boxer nicht gefunden'},404);
  const key=env.BOXING_DATA_API_KEY||env.RAPIDAPI_KEY||'';
  if(!key)return json({events:[],needsKey:true,provider:'boxing-data'});
  const fr=await fetch('https://boxing-data-api.p.rapidapi.com/v2/fighters/?name='+encodeURIComponent(b.name),{headers:{'x-rapidapi-key':key,'x-rapidapi-host':'boxing-data-api.p.rapidapi.com'}});
  if(!fr.ok)throw new Error('Boxing Data API nicht erreichbar');
  const fj=await fr.json(), fighter=(fj.data||[]).find(x=>norm(x.name||x.full_name||'')===norm(b.name))||(fj.data||[])[0];
  const fighterId=fighter?._id||fighter?.id; if(!fighterId)return json({events:[]});
  const qr=new URLSearchParams({fighter_id:String(fighterId),date_from:new Date().toISOString().slice(0,10),date_sort:'ASC',page_size:'25'});
  const rr=await fetch('https://boxing-data-api.p.rapidapi.com/v2/fights/?'+qr,{headers:{'x-rapidapi-key':key,'x-rapidapi-host':'boxing-data-api.p.rapidapi.com'}});
  if(!rr.ok)throw new Error('Box-Kämpfe konnten nicht geladen werden');
  const rj=await rr.json(), events=(rj.data||[]).filter(f=>new Date(f.date)>Date.now()-86400000).map(f=>({
    idEvent:'boxing-'+(f._id||f.id||String(f.date)),
    strEvent:f.title||((f.fighters?.red?.name||f.fighters?.fighter_1?.name||b.name)+' vs. '+(f.fighters?.blue?.name||f.fighters?.fighter_2?.name||'Gegner offen')),
    strLeague:'Boxen',strTimestamp:f.date,location:f.event?.venue||f.venue||f.event?.location||'',broadcast:f.event?.broadcaster||'',sourceUrl:'https://boxing-data.com/'
  }));
  return json({events,provider:'boxing-data'});
}if(action==='team-next'&&provider==='public-bundesliga'){const team=TEAMS.find(t=>t.id===id);if(!team)return json({error:'Verein nicht gefunden'},404);return json({events:await bundesligaEvents(team)})}return json({error:'Quelle wird noch nicht unterstützt'},400)}catch(e){return json({error:e.message||'Öffentliche Sportquelle nicht erreichbar'},e.status&&e.status<600?e.status:502)}}